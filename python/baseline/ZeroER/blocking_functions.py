...
def block_amazon_googleproducts(A, B):
    ...
    #attributes = ["title","description","manufacturer","price"]
    attributes = ["title","manufacturer"]
    C = ob.block_tables(A, B, "title", "title", word_level=True, overlap_size=1, l_output_attrs=attributes, r_output_attrs=attributes, show_progress=True, allow_missing=False)
    return C
...
#imdb.csv and dbpedia.csv
def block_imdb_dbpedia(A, B):
    ob = em.OverlapBlocker()

    l_attributes = ['title', 'starring', 'writer', 'editor']
    r_attributes = ['title', 'actor name', 'director name', 'year', 'genre']
    C2 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=2, show_progress=True, l_output_attrs=l_attributes, r_output_attrs=r_attributes)
    return C2
    
#imdb.csv and tmdb.csv
def block_imdb_tmdb(A, B):
    ob = em.OverlapBlocker()
    attributes = ['title', 'name']
    C2 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=2, show_progress=True, l_output_attrs=attributes, r_output_attrs=attributes)
    return C2
    
#imdb.csv and tvdb.csv
def block_imdb_tvdb(A, B):
    ob = em.OverlapBlocker()
...
#tmdb.csv and tvdb.csv
def block_tmdb_tvdb(A, B):
    ob = em.OverlapBlocker()
    attributes = ['title', 'name']
    C2 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=2, show_progress=True, l_output_attrs=attributes, r_output_attrs=attributes)
    return C2                
...
#rest1.csv and rest2.csv
def block_rest1_rest2(A, B):
    #assumes some preprocessing is done:
    #Specifically in half.csv : NewPrice  => Price

    id1 = 'http://www.okkam.org/ontology_restaurant1.owl#name'
    id2 = 'http://www.okkam.org/ontology_restaurant2.owl#name'    

    ob = em.OverlapBlocker()
    attributes = ['aggregate value']
    C = ob.block_tables(A, B, id1, id2, word_level=True, overlap_size=1, show_progress=True, l_output_attrs=attributes, r_output_attrs=attributes)
    return C
...
def block_walmart_amazon(A, B):
    ...
    attributes = ['title', 'modelno', 'price', 'shipweight', 'brand', 'dimensions']
    C2 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=3, l_output_attrs=attributes, r_output_attrs=attributes, show_progress=True, allow_missing=True)
...
def block_agg(A, B):
    ...
    agg = 'aggregate value'
    C2 = ob.block_tables(A, B, agg, agg, word_level=True, overlap_size=3, l_output_attrs=[agg], r_output_attrs=[agg], show_progress=True, allow_missing=True)
...
blocking_functions_mapping["rest1_rest2"] = block_rest1_rest2 #impl
blocking_functions_mapping["abt_buy"] = block_abt_buy
blocking_functions_mapping["amazon_gp"] = block_amazon_googleproducts
blocking_functions_mapping["dblp_acm"] = block_dblp_acm
blocking_functions_mapping["imdb_tvdb"] = block_imdb_tvdb #impl
blocking_functions_mapping["tmdb_tvdb"] = block_tmdb_tvdb #impl
blocking_functions_mapping["imdb_tmdb"] = block_imdb_tmdb #impl
blocking_functions_mapping["walmart_amazon"] = block_walmart_amazon #impl
blocking_functions_mapping["dblp_scholar"] = block_dblp_scholar
blocking_functions_mapping["imdb_dbpedia"] = block_imdb_dbpedia #impl
